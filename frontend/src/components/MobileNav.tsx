        ...
        <div className="absolute right-0 top-full mt-2 w-52 bg-dark-card border border-dark-border rounded-xl shadow-xl overflow-hidden z-50">
          {links.map(({ href, label, accent }) => (
            <Link
              ...
              className={`block px-4 py-3 text-sm font-medium border-b border-dark-border last:border-0 transition-colors hover:bg-dark-border/50 ${
                accent ? "text-green-400 hover:text-green-300" : "text-gray-300 hover:text-white"
              }`}
            >
              {label}
            </Link>
