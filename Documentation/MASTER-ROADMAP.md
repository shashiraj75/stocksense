...
| **004** *(proposed numbering)* | Valuation Intelligence | **Planned** | Epics 002 and 003 (both now closed) | Sector-relative/growth-adjusted valuation replaces the current static P/E cutoff, as an explainable engine. | Not started — no design study yet. Epic 003's own outcome-validation finding (growth/valuation regime interaction, [EPIC-003 Closure](Engineering-Handbook/EPICS/EPIC-003-Growth-Intelligence-Closure.md) §16) is a relevant input for scoping this Design Study, not a pre-commitment to any specific design. |
...
**In Progress:**
- Nothing is currently mid-implementation. **Epic 003 is formally closed. Epic 004 — Valuation Intelligence — is the recommended next epic**, beginning with a Design Study, informed by (not pre-committed to) Epic 003's own growth/valuation regime-interaction finding.

**Next Up:**
- Begin Epic 004's Design Study, scoping Valuation Intelligence's boundary against the current static P/E cutoff and the scattered valuation logic in `multibagger_scorecard.py`.
...
- Reuse the now-three-times-proven Data Fabric pattern (provider adapter → resolution → engine adapter → pure engine) rather than re-deriving an architecture a fourth time — confirmed by Epic 003 to transfer without modification, the same way Epic 002 confirmed it after Epic 001.
