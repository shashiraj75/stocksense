...
**Growth Intelligence — Completed (Epic 003, closed).** A dedicated, explainable growth-scoring engine is implemented and validated. It contributes a confidence-only input in India and an explainability-only output in the US. Numeric US integration remains intentionally deferred pending a future outcome-correlation re-measurement; this is accepted technical debt, not evidence that the engine is unimplemented.

**Valuation Intelligence — Completed (Epic 004, closed).** A dedicated, explainable valuation-scoring engine is implemented and validated for the scoped V1 metric set. It is integrated into `PredictionEngine` as a bounded confidence-only signal in both India and US markets, with an asymmetric +2/-4 cap and cross-engine safeguard gate. Its kill switches remain disabled by default pending a separately scoped operational activation decision; this is not evidence that the engine is unimplemented.
...
**Daily Picks Intelligence — Completed as a product feature; automated generation is live for both markets, with recorded natural-run completion evidence (2026-07-14); GitHub Actions scheduler-timing reliability is a separate, open operational concern.** Daily Picks is a live, shipped feature (top 6 BUY ideas per horizon, both markets, automated daily generation) — but architecturally it is a **consumer** of the Recommendation Intelligence pipeline above, not an independent intelligence engine. No "Daily Picks Engine" is being planned as a fourth/fifth named engine; Daily Picks will benefit automatically as the engines above mature. See `Documentation/Engineering-Handbook/Operations/Current-Release-Status.md` (Release 12B) for current evidence and the standing scheduler-reliability follow-up.
...
| **005** *(proposed numbering)* | Recommendation Consolidation Intelligence | **In Progress — frontend implementation complete; Release 13C observability deployed; Release 13D activation-readiness runbook complete; RCI flag remains disabled** | Epics 001–004 (synthesizes their existing, validated outputs — adds no new provider data) | Four validated engines' outputs are synthesized into one transparent, explainable investment thesis per stock, replacing today's opaque accumulation of small confidence nudges. | Sprints #001–#011 complete; Sprint #012 ([Live Stock Analysis Frontend Implementation](Engineering-Handbook/Releases/Sprint-012-Recommendation-Consolidation-Live-Stock-Analysis-Frontend-Implementation.md)) implements and wires in the Evidence Summary component (`frontend/src/components/EvidenceSummary.tsx` + a new `DisclosurePanel` primitive + the `RecommendationConsolidation` TypeScript contract in `api.ts`) below the AI Signal card, above the horizon tabs. **Two real findings disclosed, not worked around**: the backend has no field for a "feature-disabled engine" notice (renders nothing for that state, same as full RCI absence); the frontend has no test framework at all today, so validation used `tsc --noEmit`, a full production build, and direct script execution of the core logic against the brief's mocked scenarios, instead of a committed test suite. **Release 13C adds process-local composition-success and fail-open observability; Release 13D documents controlled activation readiness. RCI remains disabled, and any future activation is user-visible through the Evidence Summary surface rather than a backend-only dark launch.** **Recommendation acted on: Sprint #013** ([Frontend Test Foundation](Engineering-Handbook/Releases/Sprint-013-Recommendation-Consolidation-Frontend-Test-Foundation.md)) **adds the visual-QA-and-tooling sprint recommended above** — a frontend test framework (Vitest + React Testing Library, CI-wired) didn't exist at all before this sprint; 32 new tests now cover the RCI contract-validation boundary and the `DisclosurePanel`/`EvidenceSummary` components. Scope is narrow and named as such: this closes Sprint #012's specific gap, not general frontend test debt (every other frontend component remains uncovered). RCI flag still disabled; no production code changed. **The 886/886 backend-suite figure is a historical Sprint #012 snapshot; current test state must be taken from the latest validated release or CI evidence.** |
| **006** *(proposed numbering)* | Prediction and Recommendation Decision Architecture Evolution | **Future** | Epics 002–005 | The existing Prediction Engine evolves to consume Business Quality, Financial Strength, Growth, Valuation, and Risk as first-class engine inputs, rather than its current internal ad hoc factor blend. | Not started — explicitly sequenced after the component engines exist, per the same "validate the parts before integrating" discipline used throughout Epic 001. **This future architecture work is distinct from Epic 005: RCI adds an explainable, read-only consolidation layer over existing outputs, while Epic 006 would require a separately scoped decision-architecture evolution that may consume validated engines as first-class inputs.** |
...
| **008** *(proposed numbering)* | AI Research Analyst | **Future** | Epic 006/007 (needs richer, consolidated explainability data to be grounded in) | A true conversational analyst layer, distinct from today's rule-based Explainability Layer. See the consolidating [AI Equity Research Analyst — Architecture and Product Specification](Engineering-Handbook/Architecture/AI-Equity-Research-Analyst-Architecture.md), which unifies EPIC-008's safety rules with the Stock Movement Explanation Engine and the Portfolio/Daily Picks/Multibagger/Stock Detail forensic audits' independently-converged future-capability sections into one shared intelligence layer and 4-phase roadmap. | Not started — deliberately last; explicitly gated on richer explainability data existing first. |
...
**Related, more detailed specification:** [Portfolio Page — Forensic Performance & UX Audit §10 — Portfolio Daily Performance & Attribution Intelligence](Engineering-Handbook/Architecture/Portfolio-Page-Forensic-Performance-Audit.md#10-future-capability--portfolio-daily-performance--attribution-intelligence) expands this Epic's Day P&L/Day Change % scope (Sprint 001 below) into a fuller specification — per-holding daily attribution, sector/industry/country/market-cap contribution, a portfolio-level "why did this move today?" rollup of the Stock Movement Explanation Engine, a Portfolio Health Dashboard, and a persisted historical daily-snapshot timeline. **The same document's §11 ("Portfolio Copilot Vision") and §12 ("Long-Term Architecture")** extend this further into the full future Portfolio Copilot vision (health/risk review, AI recommendations, rebalancing suggestions, position sizing, diversification/correlation analysis, sector rotation, stress testing, what-if scenarios, and a future portfolio-aware AI chat gated on Epic 008) and state the governing rule that every layer (Portfolio → Intelligence Engine → Research Analyst → Portfolio Copilot) must consume, not duplicate, the layer below's already-validated output. All of this is an input to this Epic's future sprint planning, not a replacement for the sprint sequence below, and is explicitly sequenced as a Phase 2 enhancement after [Sprint 011 — Performance, Scalability & User Experience](Engineering-Handbook/Architecture/Performance-Scalability-UX-Sprint-011-Spec.md) (whose own §20.1 specifies the caching/refresh/API architecture this depends on).
...
### Planned Cross-Cutting Initiative — Sprint 011: Performance, Scalability & User Experience

**Status: Planned / Not Started. Roadmap/documentation only — no implementation, configuration, or infrastructure change has been made.** A full functional specification exists: [Sprint 011 — Performance, Scalability & User Experience: Functional Specification](Engineering-Handbook/Architecture/Performance-Scalability-UX-Sprint-011-Spec.md).

**Naming note.** "Sprint 011" here is a standalone, cross-cutting sprint number, not the 11th sprint of any single existing Epic's own sequence (Epic 002/003/004/005 each already have their own independently-numbered Sprint #001–#012 series) — the same kind of naming collision risk already flagged and handled for "Epic 007" in `Engineering-Handbook/Operations/Current-Release-Status.md`.

**Objective.** A whole-platform audit and improvement plan for performance, scalability, and user experience — frontend (Vercel/Next.js), backend (Railway/FastAPI), database (Supabase/Postgres), infrastructure, background jobs, and the Daily Picks/Paper Trading/Intelligence Engine subsystems specifically — following this codebase's established evidence-before-implementation discipline: no optimization claim without a measured baseline, no fix described as complete without a before/after number.

**Confirmed current gaps, named honestly rather than assumed away**: no dedicated caching layer (Redis/Memcached), no message queue or worker pool, no APM/observability tooling beyond the existing aggregate-only Debug Endpoint (Release 14B), no load-testing tooling, no mobile-specific performance baseline, and no frontend test framework — all confirmed by their absence, not assumed. The single clearest confirmed scalability blocker is that trained `meta_model` `.pkl` artifacts live on local Railway disk (SSDS-000, ROADMAP item 5.3) — they do not survive a redeploy and would not be shared across horizontally-scaled replicas, blocking any future multi-replica backend deployment until resolved.

**Phased approach (per the linked spec's Section 17):** Phase 0 (baseline measurement only, no code change) → Phase 1 (evidence-ranked prioritization) → Phase 2 (low-risk fixes) → Phase 3 (structural changes — job model, model-artifact storage, monitoring) → Phase 4 (load-testing validation). No phase is authorized by this roadmap entry or its linked spec; each requires its own separate implementation-sprint approval.

**Roadmap position and dependencies.** Not sequenced ahead of, or in place of, any currently in-progress Epic. Structural changes to Daily Picks' job model (Phase 3) must preserve every already-validated correctness guarantee (freshness guard, crash-handler-always-writes-a-record, per-ticker batch isolation) established across the Product Integrity Workstreams — this initiative optimizes speed and scale, it does not reopen already-closed correctness questions.

**Related audits, all completed under this initiative's Phase 0 frontend-audit scope:** [Portfolio Page — Forensic Performance & UX Audit](Engineering-Handbook/Architecture/Portfolio-Page-Forensic-Performance-Audit.md), [Daily Picks Page — Forensic Performance & UX Audit](Engineering-Handbook/Architecture/Daily-Picks-Page-Forensic-Performance-Audit.md), [Multibagger Page — Forensic & Scientific Audit](Engineering-Handbook/Architecture/Multibagger-Page-Forensic-Scientific-Audit.md), and [Stock Detail Page — Forensic & Scientific Audit](Engineering-Handbook/Architecture/Stock-Detail-Page-Forensic-Scientific-Audit.md). The Daily Picks audit's single lowest-risk finding — `/api/picks/status` already computes universe-quality telemetry (`universe_used`, `universe_degraded`, exclusion reasons) that the page never surfaces — requires no new backend logic and is recommended as this sprint's first scheduled Phase 1 item. Unlike Portfolio/Daily Picks, the Multibagger audit found no engineering-level performance issue — its confirmed gaps (no historical outcome validation, an unwired India Business Quality hard-gate, sector-agnostic valuation caps) are scientific/methodological, not performance work. The Stock Detail audit found the platform's strongest evidence-based, anti-fabrication recommendation logic (target/stop-loss/risk-reward) alongside its own confirmed inefficiency — an unconditional mount-time prefetch of all three horizons' full Prediction Engine output — and is assessed as the page closest to Research-Analyst-ready of all four audited. All four audits also name future, larger capabilities (Portfolio Daily Performance/Attribution/Copilot Vision; Daily Picks Selection Funnel/Research Analyst/Yesterday Comparison/Timeline/Universe Transparency; Multibagger Explainability; Stock Detail's AI Equity Research Report) explicitly sequenced as Phase 2+ work after this sprint, not authorized by any of the four audit documents.
