...
**In Progress:**
- Nothing is currently mid-implementation. **Epic 004 is formally closed. Epic 005 may begin**, conditioned on a short, separately-scoped operational decision (not a new validation sprint) about activating Valuation Intelligence's kill switches — see the EPIC-004 Closure report's own Recommendation section.

**Next Up:**
- An operational decision on activating `VALUATION_INTELLIGENCE_CONFIDENCE_ENABLED_IN`/`_US` in production, and wiring the cross-engine gate's hit-rate monitoring (validated analytically, not yet operationalized) into real telemetry — both follow-through items on Epic 004's own completed work, not new engineering questions.
