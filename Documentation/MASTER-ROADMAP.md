...
**Related audits, both completed under this initiative's Phase 0 frontend-audit scope:** [Portfolio Page — Forensic Performance & UX Audit](Engineering-Handbook/Architecture/Portfolio-Page-Forensic-Performance-Audit.md) and [Daily Picks Page — Forensic Performance & UX Audit](Engineering-Handbook/Architecture/Daily-Picks-Page-Forensic-Performance-Audit.md). The Daily Picks audit's single lowest-risk finding — `/api/picks/status` already computes universe-quality telemetry (`universe_used`, `universe_degraded`, exclusion reasons) that the page never surfaces — requires no new backend logic and is recommended as this sprint's first scheduled Phase 1 item. Both audits also name future, larger capabilities (Portfolio Daily Performance/Attribution/Copilot Vision; Daily Picks Selection Funnel/Research Analyst/Yesterday Comparison/Timeline/Universe Transparency) explicitly sequenced as Phase 2+ work after this sprint, not authorized by either audit document.
