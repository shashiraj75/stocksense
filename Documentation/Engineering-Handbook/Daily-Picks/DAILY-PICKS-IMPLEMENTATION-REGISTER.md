...
**Immediate recommended implementation order:** ~~`DP-009` + `DP-010`~~ (deployed, validation pending — see evidence below) → `DP-020` → horizon decision `DPD-001` covering `DP-001`–`DP-004` → `DP-005` → ~~`DP-017`~~ (implemented locally — see evidence below) → validation harness `DP-025`/`DP-026`. `DP-018`/`DP-019` remain open and are not implied by DP-017's completion.
...
| DP-017 | P0 | All | Retrained KMeans cluster IDs can permute while semantic regime labels remain pinned to fixed numeric IDs. | IMPLEMENTED — LOCAL | No — governed by `DPD-007` (DECIDED — SAFETY CONSTRAINT) | Fixed: `regime_cluster.py` now deterministically anchors every fitted/loaded KMeans model's cluster IDs to canonical semantic anchor centroids before use. See implementation evidence below. | Commit, push, then await a naturally scheduled retrain/load cycle for production verification. |
...
- Deployment status: Not deployed as of this writing. Pending push to `origin/main`; a normal automatic deployment may follow per repository behaviour — no manual deployment was initiated.
- Production-verification status: **PENDING.** This cannot become `PRODUCTION VERIFIED` merely because local tests pass, nor merely because the commit merges or deploys. It requires evidence that the deployed code, on a naturally scheduled retrain or model load, returns semantically consistent regime IDs — without a manual production retrain being triggered solely for this test. No retraining, Daily Picks generation, or backfill was performed in this session.
- Confirmation — no retraining triggered: `retrain_on_history()` was never called against real production history in this session; all test invocations used synthetic, monkeypatched history and a `tmp_path`-redirected `MODEL_PATH`.
- Confirmation — no model artifact manually rewritten: the repository's real `backend/services/alpha_engine/regime_kmeans.pkl` (untracked, confirmed via `git ls-files`) was never opened, read, or written by any test or by this implementation session; every test that exercises save/load uses a temporary path.
...
- Next safe action: Await a naturally scheduled regime retrain or model load in production (do not trigger manually) and inspect logs/`alpha_observations`/regime snapshots for `[regime] KMeans retrained on ... (anchored: identity=..., total_distance=...)` or equivalent evidence that anchoring ran and produced semantically consistent IDs, then update this entry's status to `MERGED` (already true once pushed) and, once live evidence is confirmed, `PRODUCTION VERIFIED`. Do not begin DP-018, DP-019, DP-020, or DP-025 under this entry.
