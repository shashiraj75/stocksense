...
- Lighthouse/Web Vitals baseline for Daily Picks, Stock Detail, Portfolio, Paper Trading, and Multibagger pages, both desktop and throttled-mobile profiles.
...
**Update — Multibagger page audit now complete (forensic and scientific, not just performance).** A dedicated audit of the Multibagger page has since been performed: see [Multibagger Page — Forensic & Scientific Audit](Multibagger-Page-Forensic-Scientific-Audit.md). Unlike the Portfolio/Daily Picks audits, this one found **no engineering-level N+1 or unmemoized-render issue** — the page is cache-driven (nightly-refresh, 60min `staleTime`, `keepPreviousData`) with zero per-row live-quote fan-out, the most efficiently-built of the three pages audited this sprint. Its confirmed gaps are scientific/methodological rather than performance-related: no historical outcome-validation exists for any of the three screens (unlike Daily Picks and Growth/Valuation Intelligence, each of which has one), and the already-validated India Business Quality Adapter's hard-gate output is a structural no-op for India Multibagger scoring today. Neither finding requires Phase 0-4 performance work in this sprint; both are tracked as their own future items (see the linked audit's §8 priority list) rather than folded into this sprint's scope.
...
## 22. Sequenced Follow-On — Multibagger Scientific Rigor and Explainability

**Status: Planned / Not Started. Documentation only.** Two distinct future workstreams for the Multibagger page are fully specified in [Multibagger Page — Forensic & Scientific Audit](Multibagger-Page-Forensic-Scientific-Audit.md):

- **Scientific rigor (audit §5/§6/§8, priorities 1-4)**: wiring the already-validated India Business Quality Adapter's hard-gate output into the IN scorecard (a structural no-op today), a Multibagger-screen Historical/Outcome Validation study mirroring Growth/Valuation Intelligence's existing methodology, sector-relative valuation bands reusing the Valuation Intelligence Engine, and resolving the undocumented 3Y-vs-5Y growth-window relationship between the SQL screen and the scorecard checklist. **None of these are performance work** — they do not belong in this sprint's own Phase 0-4 rollout (Section 17) and are tracked here only for cross-reference.
- **Explainability (audit §10)**: "why this qualifies," screen entry/exit changes, a historical qualification timeline, industry ranking, moat/management intelligence, a capital allocation score, a business quality timeline, an AI research summary, portfolio integration, daily change intelligence, and research citations. Several of these (entry/exit changes, historical timeline, daily change intelligence) share the same persisted daily-snapshot prerequisite already identified for [Daily Picks §21](#21-sequenced-follow-on--daily-picks-selection-funnel-research-analyst-history-and-universe-transparency) and [Portfolio §20.1](#201-portfolio-performance-strategy) — a future implementation should evaluate one shared snapshot/diff mechanism across all three features rather than building it three times.

Neither workstream is implemented or authorized by this document or the linked audit.

## 23. Stock Detail Performance Strategy

**Status: Planned / Not Started. Documentation only.** A dedicated forensic audit of the Stock Detail page has since been performed: see [Stock Detail Page — Forensic & Scientific Audit](Stock-Detail-Page-Forensic-Scientific-Audit.md). Its confirmed performance findings fold into this sprint's Phase 0/1 scope directly:

- **The single largest finding: an unconditional, mount-time prefetch of all three horizons' full Prediction Engine output**, not just the one being viewed (`page.tsx:174-186`) — a deliberate, documented UX tradeoff (avoiding a "stuck" feeling when switching horizon tabs) that costs 3x the full multi-engine compute per page visit. Recommended Phase 1 fix: evaluate hover/tab-focus-gated prefetching instead of unconditional mount-time prefetching, preserving the instant-switch UX at a fraction of the cost.
- **Up to 8-9 distinct API endpoints fire from one page** (quote, prediction ×3 horizons, news, stock-accuracy, factor-attribution, score-history, plus tab-gated fundamentals) — individually well-cached (each `staleTime` already matches its backend TTL, a real strength), but additive in aggregate. Recommended Phase 3 evaluation: a consolidated stock-detail aggregate endpoint, weighed against the caching benefits of today's independently-keyed React Query calls (a genuine tradeoff, not an obvious win either way).
- **`PaperTradeModal`'s on-open prediction fetch** (shared with Portfolio and Daily Picks, per those audits) recurs here too — the same lightweight signal-only endpoint recommendation already made for Portfolio (`§20.1`) would also benefit this page's Paper Trade CTA.
- Cold-cache full-page-load timing across all 8-9 calls was not exhaustively measured in the linked audit — a Phase 0 baseline task, alongside the Lighthouse/Web Vitals baseline already named for this page in Section 4 above.

## 24. Research Report Evolution

**Status: Planned / Not Started. Documentation only.** The Stock Detail page's future evolution into a full **AI Equity Research Report** — Executive Summary, Investment Thesis, Why Buy/Hold/Sell, Why Today, What Changed Since Yesterday, Target Price Methodology, Confidence Breakdown, Bull/Base/Bear Case, Risks, Catalysts, Invalidation Conditions, Peer/Sector Comparison, Fair Value/Valuation Discussion, Recommendation History, Historical Confidence, Technical/Fundamental Summary, Financial/Business/Management Quality, Capital Allocation, Research Citations, and an AI Analyst Summary — is fully specified in [Stock Detail Page — Forensic & Scientific Audit §21-§24](Stock-Detail-Page-Forensic-Scientific-Audit.md#21-future-capability--ai-equity-research-report).

**Not performance work — tracked here only for cross-reference and sequencing.** The linked audit names "Why this target?"/"Why this stop loss?" and Historical Confidence (reusing the already-fetched `score-history` endpoint) as the two lowest-risk, no-new-backend-work items, since the underlying data and logic are already fully computed — the same recurring "already computed, never surfaced" pattern found across every page audited this session (Daily Picks' Universe Transparency, Multibagger's "why this qualifies," Portfolio's AI Confidence column). Several other items (What Changed Since Yesterday, Recommendation History) share the same persisted daily-snapshot prerequisite already identified for Daily Picks §21 and Portfolio §20.1 — a future implementation should design one shared snapshot/diff mechanism across all four pages, not four independent ones. No phase of this future capability is implemented or authorized by this document or the linked audit.

## 25. Consolidated AI Equity Research Analyst Architecture

**Status: Planned / Not Started. Documentation only.** Sections 20-24 above each named a page-specific future capability (Portfolio Daily Performance/Copilot Vision, Daily Picks Research Analyst, Multibagger Explainability, Stock Detail's AI Equity Research Report) that independently converged on the same conclusion: a single, shared intelligence/explanation layer, not four separate ones. That convergence is now formalized in one consolidating architecture document: [AI Equity Research Analyst — Architecture and Product Specification](AI-Equity-Research-Analyst-Architecture.md), which unifies EPIC-008's safety rules with the Stock Movement Explanation Engine and all four page-specific future-capability sections into a single 4-phase implementation roadmap (shared snapshot/diff foundation → deterministic explainability layer → Intelligence Engine wiring → conversational Research Analyst per Epic 008). **This sprint's own §20.1/§21/§22/§23/§24 findings are direct inputs to that architecture's Phase 1/2/3** — none of them require the conversational layer (Phase 4) to deliver real value; the "already computed, never surfaced" pattern repeated across every page audited this session is precisely what Phase 2 (deterministic explainability, no LLM) resolves first. No phase of the consolidated architecture is implemented or authorized by this document or the linked architecture spec.
