...
- `_subprocess_isolation_enabled()` — the kill switch. Reads `DAILY_PICKS_SUBPROCESS_ISOLATION_ENABLED` fresh on every call (never cached at import time, matching this codebase's own kill-switch convention — e.g. Growth/Valuation Intelligence's confidence flags), rejecting anything but the literal `"1"`. Defaults OFF.
...
1. Enable `DAILY_PICKS_SUBPROCESS_ISOLATION_ENABLED=1` for one market first (India recommended — the market whose baseline climbed fastest in the design study's Sep 7-11 evidence, so the fix's effect is most visible soonest).
...
Single kill-switch env var — unsetting `DAILY_PICKS_SUBPROCESS_ISOLATION_ENABLED` (or setting anything other than `"1"`) restores the exact prior direct-call behavior with no further code change, deploy, or migration required.
