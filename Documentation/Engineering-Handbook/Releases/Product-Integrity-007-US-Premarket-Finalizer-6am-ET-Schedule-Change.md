...
- **`backend/api/routers/picks.py`'s `next_base_run_hint`/`next_premarket_run_hint` strings and the `/premarket-finalize` endpoint docstring** — these still describe the OLD schedule (`04:00 UTC`/`8:00 AM Dubai`/`~7:35 AM`/`7:30-9:00 AM`). This file carried a pre-existing, unrelated staged change that this phase was explicitly instructed to preserve exactly — including not editing it at all — so these two informational strings and one docstring could not be updated in this phase. They do not affect the actual acceptance-window logic (which lives in, and was correctly updated in, `premarket_finalizer.py`'s `in_premarket_window()`); they are stale informational text only. Updating them is a small, separate, explicitly-flagged follow-up once that file's unrelated staged change is resolved.
  **Resolved by the corrective release below.**

## 12. Correction and completion (2026-07-15 corrective release)

This section closes the one limitation named above. It changes no schedule, no decision logic, and no scheduling behavior — it is a copy/documentation-only completion of work this phase already implemented and deployed.

**Why a separate release was needed:** at the time of this phase's original implementation, `backend/api/routers/picks.py` carried a pre-existing, unrelated staged local modification (its own `next_base_run_hint`/`next_premarket_run_hint` edit, targeting the *previous* ~7:35 AM ET schedule) that this phase was explicitly instructed not to disturb. That constraint made it impossible to land this phase's own fix to the same lines in the primary working tree. The corrective release performed the equivalent fix from an isolated `git worktree` created from `origin/main`, so the protected staged file in the primary working tree was never read, staged, or modified.

**What changed in the corrective release:**
- `backend/api/routers/picks.py`:
  - The US empty-state `/daily` message no longer claims picks are "Generated at 9 AM ET (pre-market)" (never true, and predating the 3-phase split). It now truthfully states the base schedule (`06:00 UTC`) and, for US only, that the Premarket Review separately targets `~6:00 AM ET`.
  - `next_base_run_hint` corrected to `"06:00 UTC / 10:00 AM Dubai / 11:30 AM IST, Mon-Fri"`.
  - `next_premarket_run_hint` corrected to a DST-safe description of the `~6:00 AM America/New_York` target, both UTC cron candidates (10:00 UTC EDT / 11:00 UTC EST), and the `6:00-7:30 AM ET` backend acceptance window — matching what `in_premarket_window()` has enforced since this phase's original commit.
  - The `/generate` endpoint docstring no longer cites the old `12:30 UTC (~8:30 AM ET)` schedule; it now states the real `06:00 UTC` base schedule and clarifies this is the heavy/full base generation stage, distinct from the later Premarket Review.
  - The `/premarket-finalize` endpoint docstring's stated execution window is corrected from `7:30-9:00 AM America/New_York` to `6:00-7:30 AM America/New_York`, matching `in_premarket_window()`.
  - No response field was renamed; no field was added or removed; existing consumers remain backward compatible.
- `frontend/src/app/picks/page.tsx`: the Premarket Review badge previously appended `"Scheduled for 6:00 AM ET"` to every non-completed outcome, including `skipped` and `failed`. That is corrected — the schedule label is now shown only for the genuinely future-looking `pending` state; `skipped`/`failed` show their own truthful terminal label with no forward-looking schedule text, since today's window may already be closed. `pending`/`completed`/`completed_with_limited_premarket_data` behavior, and all India rendering, are unchanged.
- Tests: added assertions locking in the corrected `next_base_run_hint`/`next_premarket_run_hint` values, the corrected empty-state message, the corrected `/generate` and `/premarket-finalize` docstring content, and the corrected skipped/failed badge behavior — in `backend/tests/integration/test_premarket_finalizer_endpoint.py` and `frontend/src/app/picks/__tests__/premarketSchedule.test.ts`. Also updated the previously-stale `frontend/scripts/test-picks-premarket-copy.mjs`, which asserted the pre-`06:00 UTC` Dubai/IST base-time values and outdated badge-gating/"runs before market open" copy — a genuinely stale active test fixture, corrected to match the schedule and badge behavior this phase already shipped.
- Documentation: this section. No other historical record in this repository was rewritten; prior sections of this document, prior Product Integrity workstreams, and `CHANGELOG.md` entries remain exactly as originally written, since they accurately describe what was true at the time they were written.

**What did not change:** Daily Picks scoring, ranking, universe selection, persistence, finalizer decision logic (`_validate_payload_preflight`, `_validate_source_job_state`, `validate_base_for_finalization`), the base/finalizer cron schedules themselves (already correct since this phase's original commit), GPI-0, and Phase 1A/1A.3.

**Natural-run verification status:** unchanged by this corrective release — the 6:00 AM ET finalizer schedule remains **DEPLOYED, AWAITING NATURAL-RUN VERIFICATION**, carried forward from this phase's original implementation. This corrective release did not trigger, dispatch, or substitute for a natural run.
