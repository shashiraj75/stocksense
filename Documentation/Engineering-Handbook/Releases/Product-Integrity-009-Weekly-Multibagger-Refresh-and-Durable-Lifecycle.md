...
**Status:** Deployed to production (2026-07-16, commit `fbf2789`). A production forensic audit found this release's own schema migration silently failed to apply for two objects (the market constraint stayed US-only, the active-job index kept its old predicate) — India Multibagger was non-functional in production for the full day this was live. See [Product Integrity #010](Product-Integrity-010-Multibagger-Production-Hardening-and-Legacy-Schema-Repair.md) for the fix; that release's schedule/design details supersede this entry's.
