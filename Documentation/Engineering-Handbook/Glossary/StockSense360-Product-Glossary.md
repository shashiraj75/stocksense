...
### Recommendation Consolidation Intelligence (RCI)
An additive, explainability-focused layer that reads validated engine outputs and produces a structured investment-thesis and evidence summary. RCI does not replace the Prediction Engine's BUY/HOLD/SELL signal, confidence, targets, stop-loss logic, cache persistence, or Daily Picks ranking. It is distinct from the existing Recommendation Intelligence capability and from future Epic 006 decision-architecture evolution. Operationally, RCI remains disabled unless explicitly approved through `RCI_LIVE_STOCK_ANALYSIS_ENABLED`; see the Current Release Status register for live state.
