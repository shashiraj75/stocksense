...
- [SSDS-003 — StockSense360 Business Quality Engine](SSDS/SSDS-003-StockSense360-Business-Quality-Engine.md) — full design spec (framework, metrics, sector adaptation, output contract, validation strategy) for the engine answering "is this fundamentally an outstanding business worthy of long-term ownership?" Implemented in Sprint #004 (`backend/services/business_quality_engine.py`) — wired additively into the Prediction Engine; no consumer migrated yet. See [Sprint-004-Business-Quality-Engine.md](Releases/Sprint-004-Business-Quality-Engine.md).
...
- [Business Quality Engine — Production Readiness Validation](Architecture/Business-Quality-Engine-Production-Readiness-Validation.md) — 55-company live validation run against the engine implemented in Sprint #004. Found that 2 of the 5 reused `quality_factors.py` functions (Altman Z-Score, Sloan Accruals) never computed a real value against live yfinance data, and that the Piotroski F-Score had no sector-awareness. **Both fixed in Sprint #004a** — see [Sprint-004a-Business-Quality-Engine-Calibration.md](Releases/Sprint-004a-Business-Quality-Engine-Calibration.md) for the before/after re-validation (Altman/Accruals availability 0%→100%, "Quality Compounder" classification 0→25% of companies). **One new finding was surfaced by the fix and is reported, not yet fixed** (Altman has no financial-sector exemption of its own; a false-positive hard-gate rejection was found for 2 well-regarded non-financial companies, HON and ORCL) — still do not integrate into any production consumer until that follow-up is resolved and the full validation is re-run.
