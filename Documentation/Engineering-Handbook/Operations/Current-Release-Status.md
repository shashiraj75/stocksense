...
- **2026-09-13 update — the underlying ratchet is confirmed NOT fully resolved.** Triggered by a routine pre-check before a candidate-pool increase (mirroring Sprint #014's own precedent): real Railway metrics show **90.3% peak / 75.5% last-24h average** memory usage today, at unchanged candidate counts. Live `memory_guard` logs directly rule out this workstream's own weight_adapter suspicion (now confirmed completing in 4.9-5.8s) and confirm a separate 2026-08-24 run-end cleanup fix is also active — yet the cross-run baseline still climbs day-over-day until a container restart resets it. See [Daily Picks Cross-Run Memory Ratchet — Investigation and Subprocess Isolation Design](../Architecture/Daily-Picks-Cross-Run-Memory-Ratchet-Investigation-and-Subprocess-Isolation-Design.md) for the full evidence trail and the proposed permanent fix (subprocess-isolated generation). No code changed this update; the candidate-pool increase that triggered this check remains **not implemented**, pending the subprocess-isolation sprint.
...
**Status: PUBLICATION BEHAVIOR VERIFIED — FINAL GOVERNANCE EVIDENCE PENDING (as of 2026-08-17).** PR #70 (`feature/daily-picks-conviction-gated-publication`, three commits: `5a006498`, `0f2bbed8`, `7eaa8193`) merged via a real merge commit at `2026-08-16T19:44:24Z`, resulting `main` SHA `f9fa3678380d5d8cc92273d8843c183c6e2d20cf`. Both automatic deployment checks succeeded as a normal consequence of the merge (Vercel, Railway) — no manual deployment triggered. Post-deploy `/health` = 200; India/US Daily Picks confirmed idle and unaffected (no unexpected generation, restart, or dispatch as a side effect of the merge/deploy). Both markets' subsequent scheduled generation runs (read-only observed, never triggered) confirmed the policy live at every level checked — live API payload, live rendered page, AND direct `alpha_observations` database rows: India `last_successful_session_date` advanced to `2026-08-17` (`n_published<=3` on all 3 horizons, all published picks conviction=100, live page shows dynamic "Up to 3 qualified picks per horizon (Model Conviction ≥ 85/100)" copy and hides the legacy High Conviction toggle, published symbols/ranks/weights in the database match the live payload exactly); US likewise advanced to `2026-08-17` with the same invariants holding on all 3 horizons at every level. Scheduled natural-run publication behavior has been verified for both markets; final governance evidence remains pending because direct Railway restart/OOM/crash-loop metrics for the exact run windows were unavailable (no Railway log/metrics access tool available this session — only indirect evidence: jobs completed cleanly, `/health` stayed 200 throughout). This is the sole blocker for the "final governance evidence" portion of this status — it is NOT the policy's own correctness, which is fully confirmed. US Premarket Review's natural completion for today is separately still pending as of this check, but is explicitly NOT a second reason for this status: `backend/services/premarket_finalizer.py`'s own code (docstring + implementation, directly reviewed) proves it structurally cannot alter scoring/ranking/confidence/target/stop-loss or the published `picks[horizon]` list's membership/order/count — only additive `premarket_*` metadata fields — so it is non-blocking by proof, not by assumption. Full evidence trail in [DP-034, Daily-Picks Implementation Register](../Daily-Picks/DAILY-PICKS-IMPLEMENTATION-REGISTER.md#dp-034--conviction-gated-daily-picks-publication-policy-publication-behavior-verified--final-governance-evidence-pending).
...
- **Legacy-cache behavior confirmed immediately post-deploy (superseded by natural-run evidence below):** both markets' live `/api/picks/daily` briefly still served the pre-deployment cache (last generated 2026-08-14, 6 picks/horizon, no publication metadata) — correctly rendering the neutral "Qualified BUY picks per horizon" copy rather than a false "Up to 3 / Model Conviction ≥85/100" claim, with the legacy "High Conviction Only" toggle present and functional for that legacy payload.
- **Publication behavior confirmed (2026-08-17), across API + page + database.** Both markets' next scheduled (cron-triggered, never manually triggered; confirmed via GitHub Actions run history, `event: "schedule"`) generation ran and was confirmed read-only: India `last_successful_session_date` 2026-08-14 → 2026-08-17 (generated `2026-08-16T21:40:08Z`); US `last_successful_session_date` 2026-08-14 → 2026-08-17 (generated `2026-08-17T07:21:22Z`). For every horizon in both markets: `n_published<=3`, `n_published<=n_conviction_qualified`, `conviction_threshold==85.0`, `max_published_per_horizon==3`, `conviction_semantic` present, every published pick's conviction >=85.0 (all observed at 100) — confirmed independently via the live API payload, the live rendered `/picks` page (dynamic backend-sourced copy, legacy High Conviction toggle correctly hidden), and direct read-only SQL against the production `alpha_observations` table (published symbols/ranks/weights match the payload exactly; zero excluded candidates misclassified as published). US Premarket Review has not yet run today (separately pending, non-blocking); `premarket_finalizer.py` code evidence confirms it structurally cannot alter these invariants regardless. Railway-side restart/OOM metrics for the exact run windows were not independently accessible this session — this is the only outstanding governance-closure item — see the DP-034 register entry for the exact scope.

## DP-035 — Conviction-Gate Calibration Evidence-Integrity Caveat (2026-08-17)

**Status: IMPLEMENTED, TESTED, COMMITTED to `fix/daily-picks-conviction-gate-per-horizon-evidence` — NOT MERGED, NOT DEPLOYED (independent review required).** A fresh read-only re-query of `val_signals` (project `doxdexwjeonzigfewfva`) found the win-rate-lift claim cited for the `>=85` conviction threshold is not reproducible for ANY horizon — including medium, previously believed validated — because `val_signals.composite_score` is a different, simplified proxy field (max ~82 across all horizons) than the production `confidence` field the gate actually reads (which reaches 100). **The 85.0 threshold and 3-per-horizon cap are unchanged** (no evidence supports a different number); the fix is a truthful `conviction_semantic` caveat (backend metadata + a new small line on `/picks`) stating the win-rate correlation is not yet confirmed, tracked as "monitor and revisit." Full evidence and diff in [DP-035, Daily-Picks Implementation Register](../Daily-Picks/DAILY-PICKS-IMPLEMENTATION-REGISTER.md#dp-035--conviction-gate-calibration-evidence-integrity-finding-val_signals-vs-production-confidence-field-mismatch-2026-08-17).

## DP-036 — Short-Horizon Conviction-Gate Backtest: Real, Full-Population, Definitive Negative Result (2026-08-18)

**Status: IMPLEMENTED, TESTED, COMMITTED to `fix/daily-picks-short-horizon-conviction-caveat` — NOT MERGED, NOT DEPLOYED (independent review required).** A real walk-forward backtest (`backend/scripts/conviction_gate_backtest.py`, PR #73) run against the ACTUAL gate field (`alpha_observations.signal_confidence`, not DP-035's `val_signals` proxy), full population, 13,988 rows across both markets, for SHORT horizon (the only horizon with enough resolved data yet — medium is thin, long has ~zero until ~2026-10-13): India <85 win rate 51.5% vs >=85 win rate 52.4%; US <85 win rate 60.7% vs >=85 win rate 60.8% (with a lower average realized return in the >=85 bucket). This is a **definitive, adequately-sampled negative finding** for short horizon — stronger than DP-035's generic "unconfirmed" state. **The 85.0 threshold and 3-per-horizon cap are unchanged for every horizon, including short** (Option A — strengthened honest caveat — was chosen over Option B — removing the short-horizon gate — because the gate's original DP-034 justification included quality-floor/noise-suppression reasons independent of win-rate, so Option B's safety is not established by this evidence alone; Option B is recorded as an explicit human follow-up decision, not implemented here). Short horizon's `conviction_semantic` now reads a definitive "tested, no lift found" statement, textually distinct from medium/long's unchanged DP-035 "not yet confirmed" wording. Full evidence, decision rationale, and diff in [DP-036, Daily-Picks Implementation Register](../Daily-Picks/DAILY-PICKS-IMPLEMENTATION-REGISTER.md#dp-036--short-horizon-conviction-gate-backtest-finding-real-full-population-definitive-negative-result-2026-08-18).
