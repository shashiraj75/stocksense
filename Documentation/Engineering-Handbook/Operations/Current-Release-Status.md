...
**Status: NATURAL-RUN VERIFICATION COMPLETE — CONVICTION-GATED PUBLICATION POLICY CONFIRMED LIVE AND CORRECT IN PRODUCTION FOR BOTH INDIA AND US (verified 2026-08-17).** PR #70 (`feature/daily-picks-conviction-gated-publication`, three commits: `5a006498`, `0f2bbed8`, `7eaa8193`) merged via a real merge commit at `2026-08-16T19:44:24Z`, resulting `main` SHA `f9fa3678380d5d8cc92273d8843c183c6e2d20cf`. Both automatic deployment checks succeeded as a normal consequence of the merge (Vercel, Railway) — no manual deployment triggered. Post-deploy `/health` = 200; India/US Daily Picks confirmed idle and unaffected (no unexpected generation, restart, or dispatch as a side effect of the merge/deploy). Both markets' subsequent scheduled generation runs (read-only observed, never triggered) confirmed the policy live: India `last_successful_session_date` advanced to `2026-08-17` (`n_published<=3` on all 3 horizons, all published picks conviction=100, live page shows dynamic "Up to 3 qualified picks per horizon (Model Conviction ≥ 85/100)" copy and hides the legacy High Conviction toggle); US likewise advanced to `2026-08-17` with the same invariants holding on all 3 horizons and the same dynamic-copy/hidden-toggle behavior confirmed on the live page. Full evidence trail in [DP-034, Daily-Picks Implementation Register](../Daily-Picks/DAILY-PICKS-IMPLEMENTATION-REGISTER.md#dp-034--conviction-gated-daily-picks-publication-policy-natural-run-verification-complete).
...
- **Natural-run verification — COMPLETE (2026-08-17).** Both markets' next scheduled (cron-triggered, never manually triggered) generation ran and was confirmed read-only: India `last_successful_session_date` 2026-08-14 → 2026-08-17 (generated `2026-08-16T21:40:08Z`); US `last_successful_session_date` 2026-08-14 → 2026-08-17 (generated `2026-08-17T07:21:22Z`). For every horizon in both markets: `n_published<=3`, `n_published<=n_conviction_qualified`, `conviction_threshold==85.0`, `max_published_per_horizon==3`, `conviction_semantic` present, every published pick's conviction >=85.0 (all observed at 100). The live `/picks` page for both markets renders the dynamic backend-sourced "Up to 3 qualified picks per horizon (Model Conviction ≥ 85/100)" copy and correctly hides the legacy High Conviction toggle once policy metadata is present. Full detail in the DP-034 register entry linked above.
