...
- **Product Integrity #009 — weekly Multibagger refresh and durable lifecycle (2026-07-16).** The full-universe fundamentals refresh (previously nightly for India, daily-weekday for US) is now weekly for both markets: India Saturday 3:00 AM IST, US Sunday 3:00 AM America/New_York (two DST-candidate crons, backend-enforced local window). Both markets are now durable (`multibagger_refresh_jobs`, full queued/running/completed/failed/interrupted/expired lifecycle, RLS-enabled) with weekly-period idempotency and orphan/restart recovery. The #008 "cooperative stop" coupling between Daily Picks/the Premarket Finalizer and Multibagger was removed and replaced with a transactional heavy-workload lease (`US_YFINANCE_HEAVY` / `IN_SCREENER_HEAVY`) shared with Daily Picks generation for both markets — exactly one workload wins a conflict; neither cancels the other's already-running job. **Deployed, awaiting the first natural Saturday (India) and Sunday (US) scheduled runs.** See [Product Integrity #009](../Releases/Product-Integrity-009-Weekly-Multibagger-Refresh-and-Durable-Lifecycle.md).
