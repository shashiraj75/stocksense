...
- **Live legacy-cache behavior confirmed post-deploy:** both markets' live `/api/picks/daily` still serves the pre-deployment cache (last generated 2026-08-14, 6 picks/horizon, no publication metadata) — correctly rendering the neutral "Qualified BUY picks per horizon" copy on the live `/picks` page rather than a false "Up to 3 / Model Conviction ≥85/100" claim, with the legacy "High Conviction Only" toggle still present and functional.
- **Still explicitly not done:** natural-run production verification — the conviction-gated policy itself has not yet been observed operating against a freshly-generated (post-deploy) payload; this requires the next scheduled India/US generation runs. No manual Daily Picks generation was performed to force this.
