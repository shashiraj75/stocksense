...
- **US** (`multibagger_refresh_us.yml`, single fixed cron `0 8 * * 0` = Sunday 08:00 UTC = 3:00 AM EST / 4:00 AM EDT — Product Integrity #010 replaced #009's original two-DST-candidate design with one fixed cron) — ~5,300 common stocks via yfinance, ~5-6 hours.

Both markets are durable (`multibagger_refresh_jobs`, full lifecycle: queued/running/completed/failed/interrupted/expired), protected by a transactional heavy-workload lease (`US_YFINANCE_HEAVY` / `IN_SCREENER_HEAVY`) shared with that market's Daily Picks generation — exactly one of the two wins if they'd otherwise overlap; neither cancels the other's already-running job — and use per-symbol staging with atomic promotion into the active cache only once the full universe is traversed (Product Integrity #010), so a partial or failed run never exposes half-updated data. The backend accepts a scheduled call any time on its correct local weekday and enforces weekly-period idempotency (`scheduled_period_key`), so a duplicate GitHub Actions dispatch is a safe no-op rather than a duplicate run. See Product Integrity #009 and #010.
