...
def get_signal_summary(df: pd.DataFrame) -> dict:
    ...
    # --- Trend regime gate (2026-08-24, isolated PR B) ─────────────────────
    # Production evidence (validation_engine.py backtest, 2.9M+ signals):
    # this function unconditionally summed two contradictory rule families
    # every call — mean-reversion oscillators (RSI/Bollinger/StochRSI/
    # Williams %R/CCI: "buy oversold, sell overbought") and trend-following
    # rules (MACD/EMA/ADX: "buy strength, sell weakness"). In a trending
    # market, "overbought" from an oscillator usually just means "still
    # strong," not "about to reverse." ADX (already computed below) is the
    # standard trend-strength gauge; ADX > 25 means a real, developed trend
    # is in force. Below that threshold each oscillator's full contrarian
    # weight applies (range-bound/choppy conditions are exactly where
    # mean-reversion has a legitimate edge); at/above it, every oscillator
    # in this function is neutralized (score delta 0) rather than fighting
    # the trend signal — the `signals` list still records what each
    # oscillator technically read, tagged as suppressed, for transparency.
    # A missing or non-finite ADX fails safe to 0 (via `_safe`'s default),
    # which is NOT-trending — oscillators remain fully active rather than
    # being silently suppressed just because trend-strength data happens
    # to be unavailable.
    #
    # This PR intentionally does NOT touch SELL classification — see
    # fix/equity-sell-containment for that separate, independently
    # reviewable change. See Documentation/Engineering-Handbook/
    # Daily-Picks/DAILY-PICKS-IMPLEMENTATION-REGISTER.md for full evidence.
