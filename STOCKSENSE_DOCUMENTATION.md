...
- **`InviteHashRedirect` added to `providers.tsx`** — runs on every page load app-wide; checks `window.location.hash` for `access_token` + `type=invite`/`type=recovery`, and routes to `/auth/set-password?next=/accept-terms` if found. supabase-js's `detectSessionInUrl` (on by default) has already silently established the session from that hash by the time this runs — this just makes sure the user is sent to actually finish registering instead of being stranded on the marketing page.
